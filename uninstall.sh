...
if [ "$doUninstall" == true ]; then
	...
	if [[ "$doUninstallPCSX2" == true ]]; then
		...
		rm -rf "$HOME/.var/app/net.pcsx2.PCSX2 &>>/dev/null"
	fi
	...
	if [[ "$doUninstallCemu" == true ]]; then
		...
		#rm -f "$HOME/.var/app/info.cemu.Cemu &>> /dev/null"
		rm -f "$HOME/Emulation/roms/wiiu/*" &>>/dev/null
		rm -f /run/media/mmcblk0p1/Emulation/roms/wiiu/* &>>/dev/null
	fi
	#if [[ "$doUninstallXenia" == true ]]; then
	#	rm -f "$HOME/Emulation/roms/xbox360/*" &>> /dev/null
	...
	if [[ "$doUninstallESDE" == true ]]; then
		rm -rf "$HOME/.emulationstation" &>>/dev/null
	fi
