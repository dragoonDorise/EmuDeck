...
if [[ -f "$HOME/homebrew/services/PluginLoader" ]] ; then
	...
	if [ $ans -eq 0 ]; then
		...
		chmod +x "$HOME/.config/EmuDeck/user_install_script.sh"
		"$HOME/.config/EmuDeck/user_install_script.sh"
...
if [ $ans -eq 0 ]; then
	...
	if find "$romsPath/remoteplay" -type f -name "*.sh" | grep -q .; then
		...
		if [[ "$RPUninstall" != *"chiaki-ng"* ]]; then
			rm -f "$HOME/.config/EmuDeck/Emulators/chiaki-ng.AppImage" &> /dev/null
		...
		if [[ "$RPUninstall" != *"Greenlight"* ]]; then
			rm -f "$HOME/.config/EmuDeck/Emulators/Greenlight.AppImage" &> /dev/null
		...
		if [[ "$RPUninstall" != *"ShadowPC"* ]]; then
			rm -f "$HOME/.config/EmuDeck/Emulators/ShadowPC.AppImage" &> /dev/null
	...
	if find "$romsPath/generic-applications" -type f -name "*.sh" | grep -q .; then
		...
		if [[ "$GAUninstall" != *"Heroic Games Launcher"* ]]; then
			rm -f "$HOME/.config/EmuDeck/Emulators/Heroic-Games-Launcher.AppImage" &> /dev/null
	...
	if [[ "$doUninstallBigPEmu" == true ]]; then
		rm -rf $HOME/.config/EmuDeck/Emulators/BigPEmu &>> /dev/null
	...
	if [[ "${doUninstallCemuNative}" == "true" ]]; then
		rm -rf $HOME/.config/EmuDeck/Emulators/Cemu*.AppImage &> /dev/null
	...
	if [[ "$doUninstallCitra" == true ]]; then
		...
		rm -rf $HOME/.config/EmuDeck/Emulators/citra-qt*.AppImage &> /dev/null
	...
	if [[ "$doUninstallLime3DS" == true ]]; then
		...
		rm -rf $HOME/.config/EmuDeck/Emulators/lime3ds-gui*.AppImage &> /dev/null
	fi
	...
	if [[ "$doUninstallMGBA" == true ]]; then
		rm -rf $HOME/.config/EmuDeck/Emulators/mGBA.AppImage &> /dev/null
	...
	if [[ "$doUninstallPCSX2" == true ]]; then
		rm -rf $HOME/.config/EmuDeck/Emulators/pcsx2-Qt.AppImage &> /dev/null
	...
	if [[ "$doUninstallRPCS3" == true ]]; then
		...
		rm -rf $HOME/.config/EmuDeck/Emulators/rpcs3.AppImage &> /dev/null
	fi
	if [[ "$doUninstallShadPS4" == true ]]; then
		...
		rm -rf $HOME/.config/EmuDeck/Emulators/shadPS4-qt.AppImage &> /dev/null
	fi
	if [[ "$doUninstallRyujinx" == true ]]; then
		rm -rf $HOME/.config/Ryujinx &> /dev/null
		rm -rf $HOME/.config/EmuDeck/Emulators/publish &> /dev/null
	...
	if [[ "$doUninstallShadPS4" == true ]]; then
		...
		rm -rf $HOME/.local/share/applications/ShadPS4.desktop &> /dev/null
		rm -rf $HOME/.config/EmuDeck/Emulators/shadps4.AppImage &> /dev/null
	fi
	...
	if [[ "$doUninstallVita3K" == true ]]; then
		rm -rf $HOME/.config/EmuDeck/Emulators/Vita3K &> /dev/null
	...
	if [[ "$doUninstallYuzu" == true ]]; then
		...
		rm -rf $HOME/.config/EmuDeck/Emulators/yuzu.AppImage &> /dev/null
	...
	rm -rf $HOME/.config/EmuDeck/Emulators/EmuDeck.AppImage &> /dev/null
	rm -rf $HOME/.config/EmuDeck/Emulators/EmuDeck_SaveSync.AppImage &> /dev/null
	rm -rf $HOME/.config/EmuDeck/Emulators/RemotePlayWhatever.AppImage &> /dev/null
	...
	rm -rf $HOME/.config/EmuDeck/Emulators/EmulationStation-DE.AppImage &> /dev/null
	rm -rf $HOME/.config/EmuDeck/Emulators/ES-DE.AppImage &> /dev/null
