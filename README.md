...
| Remote Play Clients       | (Chiaki, Moonlight, Parsec, Greenlight)       | .sh (flatpak)                                                       |                                                                                                                                                                                                                           |
...
| Sega Dreamcast            | Retroarch FlyCast Core<br>Flycast Standalone  | .7z .cdi .chd .cue .gdi .m3u                                        | dc/dc\_boot.bin                                                                                                                                                                                                           |
...
* `steamCategory`:
    ...
    * Non-Default Parsers:
        * `COMPANYNAME CONSOLENAME`
            * If the RetroArch core's name is identical to the Standalone emulator name, add `RetroArch` in front of the `CORENAME`
            * If the standalone emulator name is identical to the RetroArch core name, add `(Standalone)` behind the `EMULATORNAME`
    * A few examples: 
        * Default Parsers:  
