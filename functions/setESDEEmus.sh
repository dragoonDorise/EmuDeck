...
setESDEEmus() {
	emu=$1
	system=$2
	FILE="$HOME/.emulationstation/gamelists/$system/gamelist.xml"
	if [ ! -f "$FILE" ]; then
		mkdir -p "$HOME/.emulationstation/gamelists/$system" && cp "$HOME/dragoonDoriseTools/EmuDeck/configs/emulationstation/gamelists/$system/gamelist.xml" "$FILE"
	else
