...
function uninstallEmuAI() {
    ...
    echo "Uninstalling $name. Deleting "$HOME/.config/EmuDeck/Emulators/$filename.$format". Deleting "$HOME/.local/share/applications/$name.desktop""

    rm -rf "$HOME/.config/EmuDeck/Emulators/$filename.$format"
