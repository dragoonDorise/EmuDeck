...
configESDE() {
	...
	if [[ ! -f "$es_systemsFile" || $reset == "true" ]]; then
		cp "$HOME/dragoonDoriseTools/EmuDeck/configs/emulationstation/custom_systems/es_systems.xml" "$es_systemsFile"
	fi
	...
	if [[ ! -f "$es_settingsFile" || $reset == 'reset' ]]; then
		cp "$HOME/dragoonDoriseTools/EmuDeck/configs/emulationstation/es_settings.xml" "$es_settingsFile"
	fi
