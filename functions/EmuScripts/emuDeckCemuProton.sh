...
CemuProton_uninstall () {
	...
	{
		find "${romsPath}/wiiu" -mindepth 1 \( -name roms -o -name mlc01 \) -prune -o -exec rm -rf '{}' \;
		rm -f "${HOME}/.local/share/applications/Cemu (Proton).desktop"
 	} &> /dev/null
}
...
# Add a Steam profile for Cemu Proton
CemuProton_addSteamInputProfile () {
