...
Ryujinx_setupSaves(){
    ...
    if [ -d "${emulationPath}/saves/ryujinx/save" ]; then
        rm -rf "${emulationPath}/saves/ryujinx/save"
        rm -rf "${emulationPath}/saves/ryujinx/saveMeta"
    fi
    
    ln -sn "$HOME/.config/Ryujinx/bis/user/save" "${emulationPath}/saves/ryujinx/save"
