...
Citra_setupStorage(){
	...
	if [ ! -d "$storagePath/citra/nand" ] && [ -d "$HOME/.var/app/org.citra_emu.citra" -o -d "$HOME/.local/share/citra-emu" ]; then
		...
		echo -e ""
		setMSG "Moving Citra states to the Emulation/storage folder"
		echo -e ""
...
Citra_setupSaves(){
	...
	linkToSaveFolder citra states "$HOME/.local/share/citra-emu"
}

#Set up textures

Citra_setupTextures(){
	mkdir -p "$HOME/.local/share/citra-emu/textures"
	linkToTexturesFolder citra textures "$HOME/.local/share/citra-emu/textures"
	
}

Citra_setupCheats(){
	linkToCheatsFolder citra cheats "$HOME/.local/share/citra-emu/cheats"
