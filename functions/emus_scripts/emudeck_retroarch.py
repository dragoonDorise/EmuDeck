...
if system == "linux":
    ...
    retroarch_assets_setting="/app/share/libretro/assets/"
if system.startswith("win"):
    ...
    retroarch_assets_setting=Path(f"{retroarch_dir}/assets")
if system == "darwin":
...
retroarch_cheats_path=Path(f"{storage_path}/retroarch/cheats")
...
def retroarch_setup_cfg_file_paths():
    ...
    sed("ASSETSFOLDER", retroarch_assets_setting, retroarch_cfg_file)
...
def retroarch_set_core_setting(config_name: str,
    ...
    if str(value) == "ED_RM_LINE":
        line_pattern = re.compile(rf"^\s*{re.escape(key)}\s*=.*$\n?", re.M)
        for cfg_path in configs:
            text = cfg_path.read_text(encoding="utf-8")
            stripped = line_pattern.sub("", text)
            if stripped != text:
                cfg_path.write_text(stripped, encoding="utf-8")
                print(f"Deleted {key} from {cfg_path!s}")
        return
...
def retroarch_update_assets(assets_url: str, assets_dir: Path, check_dir: Path = None) -> bool:
    assets_dir.mkdir(parents=True, exist_ok=True)

    check_dir = Path(check_dir) if check_dir else assets_dir
    ...
    return True
def retroarch_setup_bios_readmes():
    readmes = {
        "mame/bios": "Put your MAME bios here",
        "dc": "Put your Dreamcast bios here",
        "neocd": "Put your Neo Geo CD bios here",
    }

    for folder, text in readmes.items():
        target = Path(bios_path) / folder
        target.mkdir(parents=True, exist_ok=True)
        (target / "readme.txt").write_text(text + "\n", encoding="utf-8")
    ...
    Path(bios_path).mkdir(parents=True, exist_ok=True)
    (Path(bios_path) / "readme.txt").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def retroarch_buildbot_downloader():
    ...
    retroarch_update_assets(retroarch_ppsspp_url, Path(bios_path), retroarch_ppsspp_path)
    ...
    retroarch_update_assets(retroarch_overlays_url, retroarch_overlays_path,
                            retroarch_overlays_path / "borders")
...
def retroarch_PPSSPP_set_config():
    ...
    retroarch_set_core_setting('psp.cfg','PPSSPP','ppsspp_frameskiptype','"Number of frames"')
    ...
    retroarch_set_core_setting('psp.cfg','PPSSPP','ppsspp_inflight_frames','"Up to 1"')
...
def retroarch_nes_bezel_on():
    ...
    retroarch_set_core_setting('nes.cfg','Nestopia','input_overlay',f'"{retroarch_overlays_path}/pegasus/nes.cfg"')
    retroarch_set_core_setting('nes.cfg','Nestopia','input_overlay_auto_scale','"false"')
    retroarch_set_core_setting('nes.cfg','Nestopia','input_overlay_enable','"true"')
    retroarch_set_core_setting('nes.cfg','Nestopia','input_overlay_opacity','"0.700000"')
    retroarch_set_core_setting('nes.cfg','Nestopia','input_overlay_scale_landscape','"1.070000"')
    retroarch_set_core_setting('nes.cfg','Nestopia','input_overlay_aspect_adjust_landscape','"0.100000"')
...
def retroarch_SameBoy_gb_set_config():
    ...
    retroarch_set_core_setting('gb.cfg','SameBoy','gambatte_gb_internal_palette','"GB - DMG"')
    retroarch_set_core_setting('gb.cfg','SameBoy','gambatte_gb_link_mode','"Not Connected"')
    ...
    retroarch_set_core_setting('gb.cfg','SameBoy','gambatte_gb_palette_twb64_1','"TWB64 001 - Aqours Blue"')
    retroarch_set_core_setting('gb.cfg','SameBoy','gambatte_gb_palette_twb64_2','"TWB64 101 - 765PRO Pink"')
...
def retroarch_SameBoy_gbc_set_config():
    ...
    retroarch_set_core_setting('gbc.cfg','SameBoy','gambatte_gbc_color_correction','"GBC only"')
...
def retroarch_Gambatte_setup_core_opt():
    ...
    retroarch_set_core_setting('Gambatte.opt','Gambatte','gambatte_gb_palette_twb64_1','"TWB64 001 - Aqours Blue"')
