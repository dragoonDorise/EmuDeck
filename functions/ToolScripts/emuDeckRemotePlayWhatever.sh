...
RemotePlayWhatever_init(){			
	...
	echo -e "OK!"
	 zenity --question --width 450 --title "Close Steam/Steam Input?" --text "Now we will exit steam and launch Steam Rom Manager so you can add RemotePlayWhatever. Desktop controls will temporarily revert to touch/trackpad/L2/R2." && (kill -15 $(pidof steam) && cp $HOME/emudeck/backend/configs/steam-rom-manager/userData/userConfigurationsRPW.json $HOME/.config/steam-rom-manager/userData/userConfigurations.json && /run/media/mmcblk0p1/Emulation/tools/srm/Steam-ROM-Manager.AppImage && cp $HOME/emudeck/backend/configs/steam-rom-manager/userData/userConfigurations.json $HOME/.config/steam-rom-manager/userData/userConfigurations.json)
