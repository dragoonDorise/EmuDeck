...
echo "SERVICE - START" > $HOME/.config/EmuDeck/logs/CloudWatcher.log
...
echo "SERVICE - declare" >> $HOME/.config/EmuDeck/logs/CloudWatcher.log
...
do
  ...
  for dir in "${!current_hashes[@]}"; do
    #echo -ne "." >> $HOME/.config/EmuDeck/logs/CloudWatcher.log
    ...
    #   echo "$dir - ${current_hashes[$dir]}" >> $HOME/.config/EmuDeck/logs/CloudWatcher.log
    #   echo "$dir - $new_hash" >> $HOME/.config/EmuDeck/logs/CloudWatcher.log
    # fi
    ...
    # echo $currentEmu >> $HOME/.config/EmuDeck/logs/CloudWatcher.log
    # echo $dir >> $HOME/.config/EmuDeck/logs/CloudWatcher.log

    if [ "${current_hashes[$dir]}" != "$new_hash" ] && [[ $dir == *"$currentEmu"* ]]; then
      # Show the name of the folder immediately behind "saves"
       echo "SERVICE - CHANGES DETECTED on $dir, LETS CHECK IF ITS A DUPLICATE" >> $HOME/.config/EmuDeck/logs/CloudWatcher.log
       timestamp=$(date +%s)

       if [ $((timestamp - lastSavedTime)) == 0 ]; then
        echo "SERVICE - IGNORED, same timestamp" >> $HOME/.config/EmuDeck/logs/CloudWatcher.log
       fi
       echo $((timestamp - lastSavedTime)) >> $HOME/.config/EmuDeck/logs/CloudWatcher.log

      if [ $((timestamp - lastSavedTime)) -ge 1 ]; then
        emuName=$(get_parent_folder_name "$dir")
        #cloud_sync_update

        echo "SERVICE - $emuName CHANGES CONFIRMED" >> $HOME/.config/EmuDeck/logs/CloudWatcher.log
        echo $timestamp > "$savesPath/$emuName/.pending_upload"
        echo "SERVICE - UPLOADING" >> $HOME/.config/EmuDeck/logs/CloudWatcher.log
        show_notification "Uploading from $emuName"
        cloud_sync_uploadEmu $emuName
        rm -rf "$savesPath/$emuName/.pending_upload"
        echo "SERVICE - UPLOADED!" >> $HOME/.config/EmuDeck/logs/CloudWatcher.log
  ...
  if [ ! -f "$savesPath/.gaming" ]; then
    echo "SERVICE - NO GAMING" >> $HOME/.config/EmuDeck/logs/CloudWatcher.log
    show_notification "Uploading... don't turn off your device"
    if [ ! -f "$HOME/.config/EmuDeck/cloud.lock" ]; then
      echo "SERVICE - STOP WATCHING" >> $HOME/.config/EmuDeck/logs/CloudWatcher.log
      show_notification "Sync Completed! You can safely turn off your device"
      rm -rf "$savesPath/.watching"
      rm -rf "$savesPath/.emuName"
      echo "SERVICE - NO LOCK - KILLING SERVICE" >> $HOME/.config/EmuDeck/logs/CloudWatcher.log
...
echo "SERVICE - END" >> $HOME/.config/EmuDeck/logs/CloudWatcher.log
