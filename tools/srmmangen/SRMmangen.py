...
# say hello
print('<SRM manifest.json generator 0.2 (c) 2023 d0k3>')
...
parser.add_argument('-g', '--gamelist-root', type=str, help='specify gamelist root path (can be same as roms root)')
parser.add_argument('-o', '--output', type=str, default='./manifest.json', help='specify output destination for manifest.json')
parser.add_argument('-i', '--import-srm-parsers', type=str, help='specify SRM userConfigurations.json to import (alpha!)')
...
# mssing gamelist root? take it over from roms_root
...
if args.import_srm_parsers:
        with open(args.import_srm_parsers, 'r', encoding = 'utf-8') as fp:
            ...
            for p in srmconfig:
                ...
                e = {}
                e['subdir'] = s.group(1)
                e['emulator'] = p ['executable']['path']
                e['args'] = p['executableArgs']
