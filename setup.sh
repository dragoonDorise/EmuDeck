...
# Make the EmuDeck config directory
...
if [ -f "${PIDFILE}" ]; then
	# Get PID
	PID="$( cat "${PIDFILE}" )"
	# Check if another setup.sh is running
...
rm -rf "${HOME}/emudeck/backend"

# Create directory
emuDeckDir="${HOME}/emudeck"
if ! install -d "${emuDeckDir}"; then
	echo "Failed to create ${emuDeckDir}."
	exit 1
fi

# Custom Scripts
customScriptsDir="${HOME}/emudeck/custom_scripts"
if ! install -d "${customScriptsDir}"; then
	echo "Failed to create ${customScriptsDir}."
	exit 1
fi

# Exampe Custom Script
...
# Test Log
if ! echo "Press the button to start..." > "${LOGFILE}"; then
	exit 1
fi
...
if [ -d "${FOLDER}" ]; then
	touch "${HOME}/.config/EmuDeck/.finished"
fi
sleep 1
...
if [[ "${uiMode}" == 'zenity' || "${uiMode}" == 'whiptail' ]]; then
	...
	if [[ ! -e "${EMUDECKGIT}"/.git/config ]]; then
		if ! install -d "${EMUDECKGIT}"; then
			echo "Failed to create ${EMUDECKGIT}"
			exit 1
		fi
...
# After sourcing functions, check if emulationPath is empty.
if [[ -z "$emulationPath" ]]; then
	echo "emulationPath is Empty!"
	setMSG "There's been an issue, please restart the app"
	exit 1
fi
...
if [ -n "${PASSWD}" ]; then
	...
	echo "No password supplied. Skipping SteamGyroDSU / Powertools."
fi
...
#
touch "${HOME}/.config/EmuDeck/.finished"
touch "${HOME}/.config/EmuDeck/.ui-finished"
