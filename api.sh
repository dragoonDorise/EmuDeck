...
API_pull(){
	local branch=$1
	cd ~/.config/EmuDeck/backend && touch $HOME/.local/share/emudeck/logs/git.log && git reset --hard && git clean -fd && git checkout $branch && git pull && appImageInit && echo "OK" || echo "KO" >&2
}
